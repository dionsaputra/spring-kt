...
fun fibo1(x: Int): Int = if (x < 2) 1 else fibo1(
  x - 1
) + fibo1(x - 2)
...
fun main() {
  ...
  val upperCase =
    mapString(fullName, ::upperCaseMapper)
  println(upperCase)

  val lowerCase =
    mapString(fullName, ::lowerCaseMapper)
  println(lowerCase)

  val lowerCaseMapper = getMapper(false)
  val lowerCaseNickName =
    mapString(nickName, lowerCaseMapper)

  // TODO: lambda
  val upperCaseNickName =
    mapString(nickName) { it.toUpperCase() }
